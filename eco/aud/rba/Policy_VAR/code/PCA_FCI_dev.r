...
cCPI6 <- rollapplyr(na.ARextend(s_dat$cCPI), 6, mean)[dateFlag]
...
rbaVAR <- cbind(rba.sf_x, ref_x$cash, cCPI6)[paste0(modStartDate, '::')]
names(rbaVAR) <- c('aD', 'aFCI', 'cash', 'cCPI')
# rbaVAR.mod <- VAR(scale(rbaVAR), p=4); plot(irf(rbaVAR.mod))
rbaVAR.mod2 <- VAR(rbaVAR, p=4)
...
im <- imputePCA(coredata(pca_x), ncp=3)
...
imF <- imputePCA(coredata(fci_x), ncp=3)
...
if(purgeFCI) {
    ...
    cpi_3mma <- rollapplyr(na.ARextend(cCPI6), 3, mean)[dateFlag]
    #
...
for (i in 1:ncol(fciCmpl)) {
    resid_i <- lm(fciCmpl[,i] ~ dd1_x +
                  ...
                  na.ARextend(cCPI6[dateFlag]))$residuals
    purge1[, i] <- resid_i
}
...
#
imG <- imputePCA(coredata(grf_x), ncp=3)
...
#
SDRl3 <- rollapplyr(log(na.ARextend(allD_x$rCpixS)), 3, mean)
rCpixU3 <- rollapplyr(na.ARextend(allD_x$rCpixU), 3, mean)
reer3 <- rollapplyr(na.ARextend(allD_x$jpm_reer), 3, mean)
#
icT <- "SC"
...
# mjPCA + cCPI6 ==> rmsfe :: 3m 31.7 (30.7); 6m 95.3 (99.1); 12m 293.8 (341) -- plots are good too
MJscVAR <- cbind(sf_x, FCI_sfx, cCPI6, ref_x$cash)[modStartDateFlag]
...
# [mjPCA + cCPI6] L4 (L3) ==> rmsfe :: 3m 32.9 (37.9); 6m 98.1 (119); 12m 309 (375)
MJsRcVAR <- cbind(FCI_sfx, sf_x, cCPI6, ref_x$cash)[modStartDateFlag]
...
# [mjPCA] + {SDRl} L4 ==> rmsfe ::
MJsRcxVAR <- cbind(SDRl3, FCI_sfx, sf_x, cCPI6, ref_x$cash)[modStartDateFlag]
...
var7 <- cbind(
              log(rCpixU3), log(reer3),
              FCI_sfx, sf_x, nairu3, cCPI6, ref_x$cash)[modStartDateFlag]
#
...
dfs <- listN(rbaVAR, MJsVAR, MJscVAR,
             MJsRcVAR, MJsRcxVAR, var7,
             MJsRcVAR, MJsRcxVAR, var7)
skips <- list(165, 165, 165,
              165, 165, 165,
              165, 165, 165)
lags <- list(3, 3, 3,
             6, 6, 6,
             6, 6, 6)
icT <- list("FPE", "FPE", "FPE",
            "SC", "SC", "SC",
            "FPE", "FPE", "FPE")
rsts <- list(NULL, NULL, NULL,
             NULL, NULL, NULL,
             MJsRcRST, MJsRcxRST, var7RST)
vars <- list(frames = dfs, skip = skips, maxLag = lags, info = icT, RM = rsts)
varMods_std <- list(rbaVAR.mod2, MJsVAR.mod2, MJscVAR.mod2,
                    MJsRcVAR.mod2, MJsRcxVAR.mod2, var7.mod2,
                    MJsRcVAR.res2, MJsRcxVAR.res2, var7.res2)
varOutNames <- names(dfs)
...
makePOOScombo <- function(inMods, outName, nAhead, ff) {
    ...
    EnvN <- ls(eOut)
    largeModel <- EnvN[which.max(lapply(1:length(ls(eOut)), function(X) length(get(ls(eOut)[X], eOut))))]
...
comboPOOS6_med <- list(cash = ag6$med$cash['1999::'],
                       aD = ag6$med$aD['1999::'],
                       cCPI = ag6$med$cCPI['1999::'])
comboPOOS12 <- list(cash = ag12$mean$cash['1999::'],
                    aD = ag12$mean$aD['1999::'],
                    cCPI = ag12$mean$cCPI['1999::'])
comboPOOS12_med <- list(cash = ag12$med$cash['1999::'],
                        aD = ag12$med$aD['1999::'],
                        cCPI = ag12$med$cCPI['1999::'])
...
buildFanObj <- function(variName, confs, prd) {
    ...
    for(ci in confs) {
        ...
        fanObj[confIntRows, paste0("MML",ci)] <- get(grep(ci, confMatches, value=TRUE), confEnv)[, 'lower']
        fanObj[confIntRows, paste0("MMU",ci)] <- get(grep(ci, confMatches, value=TRUE), confEnv)[, 'upper']
    }
...
# build the objects that will be used for fancharts
objectsToBuild <- c('cash', 'cCPI')
projectionPeriods <- c(6, 12, 18, 24)
# outer loop is objectsToBuild, inner loop in projectionPeriods
lapply(objectsToBuild, function(Ob) {
       lapply(projectionPeriods,
              function(P) {
                  assign(paste0(Ob, "4c_", P),
                         buildFanObj(Ob, c(30, 60, 90), P),
                         envir = globalenv())
              }
              )
                         }
...
# gets bbg data, assigns to env, saves env and then moves it to .GlobalEnv
    ...
    bbgEnv$currentMkt <- bdp(conn, securities[-1], midFields)
    bbgEnv$currentRBA <- bdp(conn, "RBACTRD Index", "PX_LAST")
    bbgEnv$histMkt <- bdh(conn, mktRef, "PX_LAST", start_date = histStart, include.non.trading.days = FALSE)
    blpDisconnect(conn) # now close the connection
    save(bbgEnv, file = file.path(dataPATH, "audPCAbbgdataE.RData"))
    assign('bbgEnv', bbgEnv, envir = globalenv())
}

if(BBGon) {
    getBBG()
} else {
   load(file.path(dataPATH, "audPCAbbgdataE.RData"))
}

#
# unstack the historical data and add a spread
IRSx <- na.locf(bbgUnstacker(bbgEnv$histMkt))
...
# take the mid
bbgEnv$currentMkt$MID <- (bbgEnv$currentMkt[,1] + bbgEnv$currentMkt[,2])/2

# stick mids together with RBA rate
today <- as.Date(as.POSIXlt(Sys.time(), tz='Australia/Sydney'))
midList <- as.list(c(bbgEnv$currentRBA$PX_LAST, t(bbgEnv$currentMkt$MID)))
...
{
    plot.zoo(scores['2003::'][,-1], col=2:3,
             main="AUD Data Pulse: 2003--2012",
             las=1, screens=1, xlab="", ylab="", lwd=2)
...
pp_demandCombo_04p <- function()
{
    ...
    legend('topright', c('3m stack', 'monthly'), col = c(1, 3),
           lwd = c(1, 2), pch = c(19, NA), horiz=T, bg='grey94')
...
pp_fciCombo_04pCash <- function()
{
    ...
    legend('topright', c('3m stack', 'monthly', 'RBA cash'), col = c(1, 3, 2),
           lwd = c(1, 2, 2), pch = c(19, NA, NA),
           horiz=F, bg='grey94')
    mtext(text="Source: UBS, Bloomberg", side=1, line=4, adj=1)
}
...
pp_CashFanFilled <- function(fanObj, ttlPrds = NULL,
                             lineType = 's', mainString = NULL) {
    if(is.null(mainString)) {
        mainString <- "Forecast fanchart: mean / 30% / 60% / 90% conf int"
    }
    if(is.null(ttlPrds)) {
        ttlPrds <- sum(is.na(fanObj$Act)) + 4
    }
    maxRow <- nrow(fanObj)
    fanRange <- range(fanObj$MMU90, fanObj$MML90, na.rm=TRUE)
    fanDates <- seq(index(fanObj)[(maxRow - ttlPrds)], index(fanObj)[maxRow], by='mon')
    plot(coredata(fanObj$Act[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
         type = lineType, ylim = fanRange, lwd = 6, las = 1,
         xlab = '', ylab = '',
         main = mainString)
    lines(coredata(fanObj$MML90[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
          col = 'grey74')
    lines(coredata(fanObj$MMU90[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
          col = 'grey74')
    polygon(c(fanDates, fanDates[(ttlPrds + 1):1]),
            c(coredata(fanObj$MMU90[(maxRow - ttlPrds):maxRow]),
              coredata(fanObj$MML90[(maxRow - ttlPrds):maxRow])[(ttlPrds + 1):1]
              ), col = 'grey74', border = NA
            )
    lines(coredata(fanObj$MML60[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
          col = 'grey47')
    lines(coredata(fanObj$MMU60[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
          col = 'grey47')
    polygon(c(fanDates, fanDates[(ttlPrds + 1):1]),
            c(coredata(fanObj$MMU60[(maxRow - ttlPrds):maxRow]),
              coredata(fanObj$MML60[(maxRow - ttlPrds):maxRow])[(ttlPrds + 1):1]
              ), col = 'grey47', border = NA
            )
    lines(coredata(fanObj$MML30[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
          col = 'orange')
    lines(coredata(fanObj$MMU30[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
          col = 'orange')
    polygon(c(fanDates, fanDates[(ttlPrds + 1):1]),
            c(coredata(fanObj$MMU30[(maxRow - ttlPrds):maxRow]),
              coredata(fanObj$MML30[(maxRow - ttlPrds):maxRow])[(ttlPrds + 1):1]
              ), col = 'orange', border = NA
            )
    lines(coredata(fanObj$ave[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
          col = 7, lwd=4)
    if("mkt" %in% names(fanObj)) {
        lines(coredata(fanObj$mkt[(maxRow - ttlPrds):maxRow]) ~ index(fanObj)[(maxRow - ttlPrds):maxRow],
              col = 6, lwd=2, pch=5, type = 'o', cex = 1.25)
    }
    mtext(text="Source: UBS, ABS, RBA, Bloomberg", side=1, line=4, adj=1)
}

pp_CashFanFilled_6m <- function() pp_CashFanFilled(cash4c_6); pp_CashFanFilled_6m()
pp_CashFanFilled_12m <- function() pp_CashFanFilled(cash4c_12); pp_CashFanFilled_12m()
pp_CashFanFilled_24m <- function() pp_CashFanFilled(cash4c_24); pp_CashFanFilled_24m()
pp_cCPIFanFilled_6m <- function() pp_CashFanFilled(cCPI4c_6, 18, lineType = 'l'); pp_cCPIFanFilled_6m()
pp_cCPIFanFilled_12m <- function() pp_CashFanFilled(cCPI4c_12, 36, lineType = 'l'); pp_cCPIFanFilled_12m()
pp_cCPIFanFilled_24m <- function() pp_CashFanFilled(cCPI4c_24, 36, lineType = 'l'); pp_cCPIFanFilled_24m()

pp_faveVAR_mkt_6m <- function()
{
    plot.zoo(cash4c_6[(nrow(cash4c_6) - 9):nrow(cash4c_6), c('Act', 'ave', 'mkt')], screen=1,
             col = c(1, 3, 6),
...
{
    plot.zoo(cash4c_12[(nrow(cash4c_12) - 15):nrow(cash4c_12), c('Act', 'ave', 'mkt')], screen=1,
             col = c(1, 3, 6),
             type = c('s', rep('l', 2), 'o'), las=1, lwd = c(3, 2, 2),
             main = "UBS RBA cash rate model (Ave) v. mkt implied",
             xlab = "",
...
{
    plot.zoo(cash4c_6[(nrow(cash4c_6) - 9):nrow(cash4c_6), c('Act', 'ave', 'min', 'max', 'mkt')],
    ...
    legend('top',
           c('Actual', 'ave model', 'Min', 'Max', 'mkt'),
           col = c(1, 3, 4, 2, 6),
           pch = c(rep(NA, 4), 5),
           lwd = c(3, 2, 1, 1, 2),
...
{
    plot.zoo(cash4c_12[(nrow(cash4c_12) - 15):nrow(cash4c_12), c('Act', 'ave', 'min', 'max', 'mkt')],
             screen=1, col = c(1, 3, 4, 2, 6),
             type = c('s', 'l', 'l', 'l', 'o'), las=1, lwd = c(3,2,1,1,1),
             pch = c(rep(NA, 4), 5),
...
if (fromSource())
{
    ...
    plot.zoo(cash4c_6[(nrow(cash4c_6) - 9):nrow(cash4c_6),
             c('Act', 'ave', 'MML30', 'MMU30', 'MML60', 'MMU60', 'MML90', 'MMU90', 'mkt')],
             screen=1,
             col = c(1, 3, rep(c('grey10', 'grey35', 'grey80'), each = 2), 6),
    ...
    plot.zoo(cash4c_12[(nrow(cash4c_12) - 15):nrow(cash4c_12),
             c('Act', 'ave', 'MML30', 'MMU30', 'MML60', 'MMU60', 'MML90', 'MMU90', 'mkt')],
             screen=1,
             col = c(1, 3, rep(c('grey10', 'grey35', 'grey80'), each = 2), 6),
             type = c('s', rep('l', 7), 'o'), las=1, lwd = c(4,3, rep(2, 7)),
    ...
    fanchart(MJsRcxVAR.pp24, names="cash", las=1,
             main="VAR model (incl xPX): RBA policy rate")
    dev.off()

    pdf(file.path(plotPATH, "VAR_demand.pdf"))
    fanchart(MJsRcxVAR.pp24, names="aD", las=1,
             main="VAR model (incl xPX): RBA policy rate")
    dev.off()
    ...
    plot(sf_x, type = 'l', las=1,
         main = "UBS Demand Index & ABS Real GDI %QoQ", ylim = c(-3.5, 3.5))
    lines(sf_x, type = 'o', pch=19, lwd=2, col=1)
    lines(scale(diff(rGDIx, log=T)), type='l', pch=20, lwd=2, col=6)
    legend('topleft', c('UBS_Demand', 'rGDI'), col = c(1, 6),
           lwd = c(2, 2), pch = c(19, NA), horiz=F, bg = 'lightgray')
    mtext(text="Source: UBS, ABS, RBA, Bloomberg", side=1, line=4, adj=1)
    dev.off()

    pdf(file.path(plotPATH, "demandF_rGDI_06p.pdf"))
    plot(sf_x['2006::'], type = 'l', las=1,
         main = "UBS Demand Index & ABS Real GDI %QoQ", ylim = c(-3.5, 3.5))
    ...
    legend('topright', c('UBS_Demand'), col = c(1),
           lwd = c(2), pch = c(19), horiz=T, bg = 'grey94')
    mtext(text="Source: UBS, ABS, RBA, Bloomberg", side=1, line=4, adj=1)
    dev.off()

    pdf(file.path(plotPATH, "FCI.pdf"))
    plot(-1*FCI_sfx, type='o', pch=20, las=1, main="Financial Conditions Indices", ylim = c(-3, 3))
    legend('topleft', c('UBS_FCI'), col = c(1),
           lwd = c(2), pch = c(19), horiz=T, bg = 'grey94')
    mtext(text="Source: UBS, ABS, RBA, Bloomberg", side=1, line=4, adj=1)
    dev.off()

    pdf(file.path(plotPATH, "FCI_06p.pdf"))
    plot(-1*FCI_sfx['2006::'], type='o', pch=20, las=1,
         main="UBS Financial Conditions Index", ylim = c(-1.5, 3))
    legend('topright', c('UBS_FCI'), col = c(1),
           lwd = c(2), pch = c(19), horiz=T, bg = 'grey94')
    mtext(text="Source: UBS, ABS, RBA, Bloomberg", side=1, line=4, adj=1)
    dev.off()
    ...
    par(mar=c(5,4,4,4)+0.1)
    plot(-1*FCI_sfx['1999::'], type='o', pch=20, las=1,
         main="FCI v Cash Rate", ylab="+ve = tighter than average", xlab="")
    par(new=TRUE)
    ...
    mtext(text="Source: UBS, ABS, RBA, Bloomberg", side=1, line=4, adj=1)
    legend('topleft', c('UBS_FCI', 'RBAcash'),
           horiz=F, pch = c(20, NA), lwd = c(1,2), col = c(1, 2), bg = 'grey94')
    dev.off()

    pdf(file.path(plotPATH, "FCI_cash_04p.pdf"))
    par(mar=c(5,4,4,4)+0.1)
    plot(-1*FCI_sfx['2004::'], type='o', pch=20, las=1,
         main="FCI v Cash Rate", ylab="+ve = tighter than average", xlab="")
    par(new=TRUE)
    plot(coredata(ref_x$cash['2004::']) ~ index(ref_x['2004::']), type='l', col=2, xaxt='n',
    ...
    mtext("RBA Cash", side=4, line=3)
    mtext(text="Source: UBS, ABS, RBA, Bloomberg", side=1, line=4, adj=1)
    legend('topright', c('UBS_FCI', 'RBAcash'),
           horiz=F, pch = c(20, NA), lwd = c(1,2), col = c(1, 2), bg = 'grey94')
    dev.off()
    ...
    plot(GRF_sfx['1990::'], type='o', pch=20,
         main="Global Risk Factor (+ve = more risk)", las=1, major.format = "%b-%y")
    legend('bottomleft', c('UBS_GlobalRisk'), col = c(1),
           lwd = c(2), pch = c(19), horiz=T, bg='grey94')
    mtext(text="Source: UBS, Bloomberg", side=1, line=4, adj=1)
    dev.off()
    ...
    pdf(file.path(plotPATH, "medVAR_cashPOOS6.pdf"))
    plot.zoo(comboPOOS6_med$cash['1999::'],
             screen=1,
             col=c(1, rep(8, ncol(comboPOOS6_med$cash)-1)),
             las=1,
             lwd = c(3, rep(1, ncol(comboPOOS6_med$cash) - 1)),
             type = c('s', rep('l', ncol(comboPOOS6_med$cash) - 1)),
             main = "Pseudo Out of Sample forecasts: median VAR",
             xlab = "",
             ylab = ""
             )
    ...
    pdf(file.path(plotPATH, "medVAR_cashPOOS12.pdf"))
    plot.zoo(comboPOOS12_med$cash['1999::'],
             screen=1,
             col=c(1, rep(8, ncol(comboPOOS12_med$cash)-1)),
             las=1,
             lwd = c(3, rep(1, ncol(comboPOOS12_med$cash) - 1)),
             type = c('s', rep('l', ncol(comboPOOS12_med$cash) - 1)),
             main = "Pseudo Out of Sample forecasts: median VAR",
             xlab = "",
             ylab = ""
             )
    ...
    ## this is the 12m ahead POOS chart
    ...
    plot.zoo(comboPOOS12$cash['1999::'],
             ...
             main = "Pseudo Out of Sample (12m forecasts): model average",
             xlab = "",
             ylab = ""
             )
    mtext(text="Source: UBS, RBA ", side=1, line=4, adj=1)
    ...
    plot.zoo(comboPOOS6$cash['1999::'],
             ...
             main = "Pseudo Out of Sample (6m forecasts): model average",
             xlab = "",
             ylab = ""
             )
    mtext(text="Source: UBS, RBA ", side=1, line=4, adj=1)
