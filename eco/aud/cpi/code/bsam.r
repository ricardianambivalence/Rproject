...
# tradable - nonTradable twin plot -- price level
tnt_cpi <- xtsF(data.frame('date' = absT8$date[-c(1:44)],
                      'nonTrade' = absT8$CPI_nontradable[-c(1:44)],
                      'tradable' = absT8$CPI_tradable[-c(1:44)]
                      ))


gp_tntIdx <- ggplot(data = meltx(tnt_cpi['1998::']),
                    aes( x = date, y = value, color = variable)) +
                geom_line() +
                labs(y = 'Index', x = NULL) +
                labs(title = "Tradable and non-tradable price indices") +
                scale_color_manual(values = RAPal_2) +
                theme(legend.title=element_blank())
png(file.path(plotPATH, "totIdx.png"))
print(gp_tntIdx)
dev.off()

# tradable - nonTradable twin -- YoY%
tnt_cpiY <- (tnt_cpi / lag(tnt_cpi, 4)  - 1) * 100

gp_tntIdxYoY <- ggplot(data = meltx(tnt_cpiY['1998::']),
                    aes( x = date, y = value, color = variable)) +
                geom_line() +
                labs(y = NULL, x = NULL) +
                labs(title = "Tradable and non-tradable inflation: %YoY") +
                scale_color_manual(values = RAPal_2) +
                theme(legend.title=element_blank())
png(file.path(plotPATH, "tntYoY.png"))
print(gp_tntIdxYoY)
dev.off()

# tnt -- ratio stuff

tradNtradx <- xtsF(data.frame('date' = absT8$date[-c(1:44)],
...
png(file.path(plotPATH, "tntRatio.png"))
...
png(file.path(plotPATH, "tntRatio_TOT.png"))
par(mar = c(5, 4, 4, 5) + 0.1)
plot(tNt_tot['19980601::', 1], main = "Terms of Trade v. T-NT CPI Ratio",
     type = 'o', major.format = "%b-%y", las=1)
par(new = TRUE)
plot(tNt_tot['19980601::', 2], type = 'l', xaxt = 'n', yaxt = 'n', xlab = '', ylab = '',
     main = "", las=1)
lines(tNt_tot['19980601::', 2], type = 'l', col = 'blue', xaxt = 'n', yaxt = 'n',
      lwd = 2, xlab = '', ylab = '')
axis(4, las=1)
mtext('Tradable-nonTradable CPI Ratio', side = 2, line = 3)
mtext('Terms of Trade', side = 4, line = 3)
legend('topleft', c('NonT/T CPI ratio', 'Terms of Trade'),
       pch = c(1, NA), lwd = 2, col = c(1,4), horiz = TRUE, bg = 'grey91')
mtext('www.ricardianambivalence.com', side=1, line=4, adj=1)
mtext('Source: ABS', side=1, line=4, adj=0)
dev.off()
